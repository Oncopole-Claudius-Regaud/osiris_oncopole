...
def _log_debug_target_in_postgres(debug_num_doss: str | None):
    ...
    rows = pg_hook.get_records(
        """
        SELECT num_doss, jour, dat_admini, num_cycle, code_ucd, code_dci, lib_dci, lib_ucd, cp_code_voie_adm, cp_lib_med_presc, cp_code_dci, num_pdt, ce_etat_chimio
        FROM osiris.chimiotherapie
        WHERE CAST(num_doss AS TEXT) = %s
        ORDER BY dat_admini NULLS FIRST, jour NULLS FIRST, num_cycle NULLS FIRST, num_pdt NULLS FIRST
        LIMIT %s
        """,
